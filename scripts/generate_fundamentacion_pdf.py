...
def build_pdf(filename="Fundamentacion_Metodologica_Digital_Twins_inDrive.pdf"):
    ...
    # Section 5 (NEW SECTION REQUESTED BY ARIANA & USER)
    story.append(KeepTogether([
        Paragraph("5. Protocolos de Confidencialidad, Gobernanza de Datos y Seguridad en IA", h2_style),
        Paragraph("Garantizar el cumplimiento de las normativas de protección de datos y la salvaguarda de secretos comerciales de inDrive constituye un pilar estructural del sistema. A continuación se detallan los cuatro niveles de seguridad aplicados:", body_style),
        Paragraph("1. <b>Anonimización Absoluta en Origen (PII Purging Protocol):</b> Previo a cualquier procesamiento por modelos de lenguaje o ingesta en la base de datos, el 100% de la información de identificación personal (PII) —nombres de entrevistados, números de teléfono, DNI, placas y direcciones exactas— fue purgada de las transcripciones y reemplazada por códigos anonimizados estandarizados (ej. <i>[PARTICIPANTE_01]</i>).", bullet_style),
        Paragraph("2. <b>Garantía de No-Entrenamiento y Retención Cero (Zero Retention Enterprise API):</b> Las consultas procesadas mediante APIs comerciales (Google Gemini API / OpenAI API) operan bajo Términos de Servicio Empresariales (Enterprise API Privacy Terms). Los datos transmitidos <b>NO son almacenados ni utilizados para entrenar, reentrenar o calibrar modelos públicos de inteligencia artificial</b> de ningún proveedor.", bullet_style),
        Paragraph("3. <b>Motor Cualitativo Local y Fail-Safe (Procesamiento Híbrido Offline):</b> La plataforma cuenta con un motor cualitativo local de respaldo que permite procesar consultas y generar análisis directamente sobre el corpus indexado en el servidor sin necesidad de enviar información a servidores o APIs de terceros.", bullet_style),
        Paragraph("4. <b>Escalabilidad Corporativa & Despliegue Privado Dedicado (On-Premise VPS / Private VPC):</b> Para un despliegue corporativo definitivo a escala donde inDrive requiera aislamiento absoluto de su información estratégica, la arquitectura está preparada para montarse en una nube privada dedicada de la empresa (VPC/VPS en AWS, GCP o Azure), ejecutando modelos Open Source de código abierto (ej. Llama 3 / Mistral) en un entorno 100% <i>Air-Gapped</i> sin conectividad externa.", bullet_style),
        Spacer(1, 8),
